...
Reduces stretching in the selected UVs — the closest Blender equivalent to Relax in 3ds Max. It wraps **UV → Minimize Stretch**, run for a fixed number of **Iterations** (default 50) instead of interactively, so it is one click. Works on the selected UVs; pin any UVs you want to keep in place first.
...
Retiles the active checker to simulate a texture resolution: **128 / 256 / 512 / 1K / 2K / 4K / 8K**. This changes how many times the checker pattern repeats across the UV tile, matching what the real texture would look like at that resolution. Affects every tileable checker material in the scene (Gradient is excluded, since it is not meant to tile). The image shown in the UV Editor is retiled to match (a pre-tiled copy; each tile keeps the checker's native pixels up to a 2048 px image, and is scaled down beyond that, so very high sizes show a coarser pattern), so the UV Editor and the model agree; Remove Checker clears it again.
